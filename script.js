/* ===================================================
   ...
   Independence Day Edition – 50% Discount + All Features
   =================================================== */

const APP_VERSION = "3.0"; // Independence Day Version
...
// ========== INDEPENDENCE DAY DISCOUNT SETTINGS ==========
...
function openCourseModal(c) {
  ...
  courseModalContent.innerHTML = `
    ...
    <p style="font-size:1.4rem; color:var(--cyan); font-weight:700;">
      ...
      ${DISCOUNT_ACTIVE && c.fee > 0 ? `
        <span style="text-decoration:line-through; opacity:0.6; font-size:1rem; margin-left:10px;">${formatFee(c.fee)}</span>
        <span style="background:#dc2626; color:white; padding:3px 10px; border-radius:12px; font-size:0.8rem; margin-left:8px;">50% OFF</span>
      ` : ''}
    </p>
    ...
    <ul class="topic-list">${c.topics.map(t => `<li>✅ ${t}</li>`).join('')}</ul>
...
// ========== COUNTDOWN TIMER (14 AUGUST 2025) ==========
...
const statusMessages = [
  "🇵🇰 Independence Day Special – 50% Off on All Courses!",
